...
function formatNotificationStatus(notification: AlertNotificationItem): string {
  ...
  if (notification.errorCode === 'noise_suppressed') return '노이즈 억제됨';
...
const AlertsPage: React.FC = () => {
  ...
  const handleCreateRule = async (payload: AlertRuleCreateRequest) => {
    ...
    try {
      ...
      setCreateSuccess(`알림 규칙 "${created.name}"이(가) 생성되었습니다`);
      await loadRules(1);
      return true;
    } catch (error) {
  ...
  return (
    <AppPage className="space-y-5">
      <PageHeader
        ...
        title="알림 센터"
        description="이벤트 알림, 일봉 기술 지표, 관심종목, 보유/계정 연동 및 시장 신호등 규칙을 관리하고, 일회성 테스트를 실행하며, 백그라운드 평가 작업 기록의 트리거 이력을 확인합니다."
      />
      ...
      <Card title="알림 시도 기록" subtitle="알림 결과" variant="bordered" padding="md">
        ...
        {!notificationsLoading && notifications.length === 0 ? (
          <EmptyState
            ...
            title="알림 시도 기록 없음"
            description="표시할 알림 시도 내역이 없습니다. 알림 트리거는 구성된 알림 채널로 계속 전송됩니다."
          />
        ) : null}
        {!notificationsLoading && notifications.length > 0 ? (
