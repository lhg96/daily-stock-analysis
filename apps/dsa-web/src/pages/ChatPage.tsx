...
const QUICK_QUESTIONS: Array<{
  ...
  { label: '채론으로 마오타이 분석', skill: 'chan_theory', stockContext: { stock_code: '600519', stock_name: '마오타이' } },
  { label: '파동이론으로 닝더스다이 보기', skill: 'wave_theory', stockContext: { stock_code: '300750', stock_name: '닝더스다이' } },
  { label: '비야디 추세 분석', skill: 'bull_trend', stockContext: { stock_code: '002594', stock_name: '비야디' } },
  { label: '박스권 분석으로 A주 SMIC 688981 분석', skill: 'box_oscillation', stockContext: { stock_code: '688981', stock_name: 'SMIC' } },
  { label: '텐센트 hk00700 분석', skill: 'bull_trend', stockContext: { stock_code: 'HK00700', stock_name: '텐센트' } },
  { label: '감정 사이클로 동팡차이푸 분석', skill: 'emotion_cycle', stockContext: { stock_code: '300059', stock_name: '동팡차이푸' } },
];

const MAX_SELECTED_SKILLS = 3;
const CONTEXT_COMPRESSION_CONFIG_KEY = 'AGENT_CONTEXT_COMPRESSION_ENABLED';
const STRONG_COMPARE_STOCK_MESSAGE_RE = /比较|对比|\bvs\b|\bcompare\b|和[^，。,.!?！？]{0,40}比|비교|대조|대비|vs|compare/i;
const WEAK_COMPARE_STOCK_MESSAGE_RE = /差异(?!化)|区别|不同|相比|对照|比一比|차이|다름|차이점|다른 점|다르다|비교해/;
const CHOICE_COMPARE_STOCK_MESSAGE_RE = /哪个|哪只|哪一个|谁更|更值得|更适合|怎么选|选哪|二选一|어느|어떤|누가 더|더 좋은|더 나은|어떻게 선택|고르|추천해/;
const LINKED_COMPARE_STOCK_MESSAGE_RE = /(?:和|与|跟|同)[^，。,.!?！？]{0,40}(?:差异(?!化)|区别|不同|相比|对照|比一比)|(?:랑|와|과|하고)[^，。,.!?！？]{0,40}(?:차이|다름|비교|대비|어떤게|뭐가)/;
const SWITCH_STOCK_MESSAGE_RE = /换成|改看|分析|看看|研究|诊断|바꿔|바꿔서|분석해|봐줘|알려줘|진단/;
...
const ChatPage: React.FC = () => {
    ...
    async (stockCode: string) => {
      ...
      try {
        ...
        if (existingStockCode) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`관심종목에서 ${stockCode} 제거됨`);
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`관심종목에 ${stockCode} 추가됨`);
          }
        }
      } catch {
        if (isMountedRef.current) {
          setWatchlistMessage('작업에 실패했습니다. 다시 시도해 주세요.');
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || '컨텍스트 압축 설정을 불러올 수 없습니다');
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || '컨텍스트 압축 설정 저장에 실패했습니다');
      } finally {
  ...
  const handleSend = useCallback(
    async (
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : ['일반'];
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? '# 사용자 메시지' : `# AI 답변${skillLabel ? ` · ${skillLabel}` : ''}`;
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'tool_done')
      return `${last.display_name || last.tool} 완료`;
    ...
    if (last.type === 'generating')
      return last.message || '최종 분석을 생성하는 중...';
    return '처리 중...';
  };
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">생각 과정</span>
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          text = step.message || `${step.step}번째 단계: 생각`;
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          <DashboardStateBlock
            compact
            title="대화 기록 없음"
            description="질문을 시작하면 여기에 대화 기록이 보관됩니다."
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        title="대화 삭제"
        message="삭제하면 이 대화는 복구할 수 없습니다. 삭제하시겠습니까?"
        confirmText="삭제"
        cancelText="취소"
        isDanger
        ...
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content="대화를 Markdown 파일로 내보내기">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label="대화를 Markdown 파일로 내보내기"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content="설정된 알림 봇/이메일로 전송">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: '알림 채널로 전송되었습니다' }, 3000);
                        } catch (err) {
                          const parsed = getParsedApiError(err);
                          showSendFeedback({
                            type: 'error',
                            message: parsed.message || '전송 실패',
                          }, 5000);
                        } finally {
                          setSending(false);
                        }
                      }}
                      aria-label="설정된 알림 봇/이메일로 전송"
                    >
                      {sending ? (
                        <svg
          ...
          {sendToast ? (
            <InlineAlert
              ...
              title={sendToast.type === 'success' ? '전송 성공' : '전송 실패'}
                  ...
                  >
                    {msg.role === 'assistant' && (skillLabel || msg.backend) && (
                      <div className="mb-2 flex flex-wrap gap-2">
                        {skillLabel ? <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={`스킬 ${skillLabel}`}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
                      ...
                      <div className="relative">
                        <div className="chat-message-actions">
                          ...
                          <button
                            ...
                            className="chat-copy-btn"
                            aria-label="이 메시지를 Markdown으로 내보내기"
                          >
                            내보내기
                          </button>
                        </div>
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  variant="info"
                  title="후속 질문 컨텍스트를 불러오는 중"
                  message="이전 분석 컨텍스트를 불러오는 중입니다. 지금 바로 후속 질문을 보낼 수 있습니다."
                  className="rounded-xl px-3 py-2 text-xs shadow-none"
                />
              ) : null}
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  <span className="text-xs text-muted-text">긴 대화의 토큰 절약</span>
                </label>
              ...
              {skills.length > 0 && (
                <div className="space-y-2">
                  <button
                    ...
                    aria-label={mobileSkillPickerOpen ? '전략 선택 접기' : '전략 선택 펼치기'}
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="flex-shrink-0 font-medium">전략</span>
            ...
            {activeStockCode && (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-[11px]"
                >
                  {stockInWatchlist(activeStockCode) ? '관심종목에서 삭제' : '관심종목에 추가'}
                </Button>
                {watchlistMessage && (
              ...
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  placeholder="예: 600519 분석 / 마오타이 지금 매수해도 될까요? (Enter 전송, Shift+Enter 줄바꿈)"
