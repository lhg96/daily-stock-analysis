...
const FALLBACK_BROKERS: PortfolioImportBrokerItem[] = [
  { broker: 'huatai', aliases: [], displayName: '화타이' },
  { broker: 'citic', aliases: ['zhongxin'], displayName: '중신' },
  { broker: 'cmb', aliases: ['cmbchina', 'zhaoshang'], displayName: '자오상' },
];
...
type PortfolioPageLanguage = 'ko' | 'en';
...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('브로커 목록 API가 빈 값을 반환하여 내장 브로커 목록(화타이/중신/자오상)으로 대체했습니다.');
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning('브로커 목록 API를 사용할 수 없어 내장 브로커 목록(화타이/중신/자오상)으로 대체했습니다.');
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || '위험 데이터를 가져오지 못해 스냅샷 데이터만 표시합니다.');
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(`${row.symbol} 분석 작업 제출: ${task.taskId}`);
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning('오른쪽 위에서 특정 계좌를 먼저 선택한 후 입력 또는 가져오기를 제출하세요.');
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning('오른쪽 위에서 특정 계좌를 먼저 선택한 후 삭제 수정을 진행하세요.');
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning('특정 계좌를 먼저 선택한 후 포지션 계좌를 삭제하세요.');
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError('계좌 이름은 비워둘 수 없습니다.');
    ...
    try {
      ...
      setAccountCreateSuccess('계좌가 생성되어 해당 계좌로 자동 전환되었습니다.');
    } catch (err) {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">{text.createAccount}</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-xs text-secondary">생성 후 자동으로 해당 계좌로 전환</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            >
              <option value="cn">시장: A주 (cn)</option>
              <option value="hk">시장: 홍콩 (hk)</option>
              <option value="us">시장: 미국 (us)</option>
              <option value="jp">시장: 일본 (jp)</option>
              <option value="kr">시장: 한국 (kr)</option>
              <option value="tw">시장: 대만 (tw)</option>
            </select>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="종목 코드 (예: 005930)" value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="체결가 (필수)" value={tradeForm.price}
            ...
            <p className="text-xs text-secondary">수수료와 세금은 비워둘 수 있으며 0으로 처리됩니다.</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>거래 제출</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">수동 입력: 자금 내역</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={cashForm.direction}
                ...
                <option value="in">유입</option>
                <option value="out">유출</option>
              </select>
            </div>
            ...
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={`통화 (선택, 기본 ${writableAccount?.baseCurrency || '계좌 기준 통화'}）`} value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))} />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>자금 내역 제출</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">수동 입력: 기업 이벤트</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={corpForm.actionType}
                ...
                <option value="split_adjustment">액면분할/병합 조정</option>
              </select>
            </div>
            ...
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>기업 이벤트 제출</button>
          </form>
        </Card>
      </section>
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  ...
                  <option value="huatai">huatai (화타이)</option>
                )}
              </select>
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">미리보기만 (쓰기 안 함)</label>
            </div>
            ...
            {csvParseResult ? (
              <InlineAlert
                ...
                title="CSV 파싱 결과"
                message={`유효 ${csvParseResult.recordCount}건, 건너뜀 ${csvParseResult.skippedCount}건, 오류 ${csvParseResult.errorCount}건.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                message={`${csvDryRun ? '미리보기 확인' : '실제 기록'}: 기록 ${csvCommitResult.insertedCount}건, 중복 ${csvCommitResult.duplicateCount}건, 실패 ${csvCommitResult.failedCount}건.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder="종목 코드로 필터링" value={eventSymbol}
            ...
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">전체 기업 이벤트</option>
                <option value="cash_dividend">현금 배당</option>
                <option value="split_adjustment">액면분할/병합 조정</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? '삭제 수정은 단일 계좌 보기에서만 가능합니다. 먼저 특정 계좌를 선택한 후 잘못된 내역을 삭제하세요.' : '잘못된 내역이 있으면 삭제 후 다시 입력할 수 있습니다.'}
            </div>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: `${item.tradeDate}의 ${formatSideLabel(item.side)} 내역 ${item.symbol} 삭제 확인 (수량 ${item.quantity}, 가격 ${item.price})?`,
                      })}
                    >
                      삭제
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: `${item.eventDate} 자금 내역 삭제 확인 (${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency})?`,
                      })}
                    >
                      삭제
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `${item.effectiveDate} 기업 이벤트 삭제 확인 ${formatCorporateActionLabel(item.actionType)} (${item.symbol})?`,
                      })}
                    >
                      삭제
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title="내역 없음"
                      description="필터 조건을 조정하거나 거래, 자금, 기업 이벤트를 먼저 입력하세요."
                      className="border-none bg-transparent px-3 py-6 shadow-none"
                    />
                  ) : null}
            </div>
            <div className="flex items-center justify-between text-xs text-secondary">
              <span>{eventPage} / {totalEventPages} 페이지</span>
              <div className="flex gap-2">
      ...
      <ConfirmDialog
        ...
        title="잘못된 내역 삭제"
        message={pendingDelete?.message || '이 내역을 삭제하시겠습니까?'}
