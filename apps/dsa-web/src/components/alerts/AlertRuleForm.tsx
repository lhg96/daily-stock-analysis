...
const SYMBOL_ALERT_TYPE_OPTIONS = [
  ...
  { value: 'ma_price_cross', label: '가격 이동평균 돌파' },
  { value: 'rsi_threshold', label: 'RSI 임계값' },
  { value: 'macd_cross', label: 'MACD 골든크로스/데드크로스' },
  { value: 'kdj_cross', label: 'KDJ 골든크로스/데드크로스' },
...
const PORTFOLIO_ALERT_TYPE_OPTIONS = [
  ...
  { value: 'portfolio_drawdown', label: '포트폴리오 드로다운' },
  { value: 'portfolio_price_stale', label: '포트폴리오 가격 상태' },
];

const MARKET_ALERT_TYPE_OPTIONS = [
  { value: 'market_light_status', label: '시장 신호등 상태' },
  { value: 'market_light_score_drop', label: '시장 신호등 점수 하락' },
];

const TARGET_SCOPE_OPTIONS = [
  { value: 'single_symbol', label: '단일 종목' },
  { value: 'watchlist', label: '관심 종목' },
  { value: 'portfolio_holdings', label: '보유 종목' },
  { value: 'portfolio_account', label: '보유 계좌' },
  { value: 'market', label: '전체 시장' },
];
      ...
      <form className="space-y-4" noValidate onSubmit={(event) => void handleSubmit(event)}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Select
            ...
            value={targetScope}
            options={language === 'ko' ? TARGET_SCOPE_OPTIONS : ALERT_TARGET_SCOPE_OPTIONS[language]}
            disabled={isSubmitting}
          ...
          <Select
            label={text.severity}
            value={severity}
            options={language === 'ko' ? SEVERITY_OPTIONS : ALERT_SEVERITY_OPTIONS[language]}
            disabled={isSubmitting}
        ...
        {alertType === 'price_cross' ? (
          <div className="grid gap-4 md:grid-cols-2">
            <Select
              ...
              options={language === 'ko' ? PRICE_DIRECTION_OPTIONS : ALERT_PRICE_DIRECTION_OPTIONS[language]}
              disabled={isSubmitting}
        ...
        {alertType === 'price_change_percent' ? (
          <div className="grid gap-4 md:grid-cols-2">
            <Select
              ...
              options={language === 'ko' ? CHANGE_DIRECTION_OPTIONS : ALERT_CHANGE_DIRECTION_OPTIONS[language]}
              disabled={isSubmitting}
        ...
        {alertType === 'ma_price_cross' ? (
          <div className="grid gap-4 md:grid-cols-2">
            <Select
              ...
              options={language === 'ko' ? THRESHOLD_DIRECTION_OPTIONS : ALERT_THRESHOLD_DIRECTION_OPTIONS[language]}
              disabled={isSubmitting}
        ...
        {alertType === 'macd_cross' ? (
          <div className="grid gap-4 md:grid-cols-4">
            <Select
              ...
              options={language === 'ko' ? CROSS_DIRECTION_OPTIONS : ALERT_CROSS_DIRECTION_OPTIONS[language]}
              disabled={isSubmitting}
        ...
        {alertType === 'portfolio_stop_loss' ? (
          <Select
            ...
            value={stopLossMode}
            options={language === 'ko' ? STOP_LOSS_MODE_OPTIONS : ALERT_STOP_LOSS_MODE_OPTIONS[language]}
            disabled={isSubmitting}
        ...
        {alertType === 'market_light_status' ? (
          <div className="space-y-2">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {(language === 'ko' ? MARKET_LIGHT_STATUS_OPTIONS : ALERT_MARKET_LIGHT_STATUS_OPTIONS[language]).map((option) => (
                <Checkbox
                  key={option.value}
                  label={option.label}
