...
export const UiLanguageToggle: React.FC<UiLanguageToggleProps> = ({
  ...
  const label = language === 'ko' ? t('language.uiLanguage') : t('language.current');
  ...
  return (
    <div className={cn('relative', isRailVariant ? 'w-full' : '', wrapperClassName)}>
      ...
      >
        ...
        {isRailVariant ? (
          <span className={labelClassName}>{language === 'ko' ? t('language.short.zh') : t('language.short.en')}</span>
        ) : isNavVariant ? (
