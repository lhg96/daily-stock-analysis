...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  { value: 'json', label: 'JSON', hint: 'response_format JSON 출력이 가능한지 검사합니다.' },
  { value: 'tools', label: 'Tools', hint: 'function/tool calling이 가능한지 검사합니다.' },
  { value: 'stream', label: 'Stream', hint: '스트리밍 출력이 유효한 chunk를 반환하는지 검사합니다.' },
  { value: 'vision', label: 'Vision', hint: '현재 모델이 image_url 입력을 지원하는지 검사합니다.' },
];
...
const RUNTIME_ONLY_HERMES_SECRET_MESSAGE = '런타임에 주입된 Hermes Key는 다시 전송되지 않습니다. 설정 페이지에서 테스트하려면 Key를 다시 입력하거나 .env에 저장하세요.';
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-[11px] text-secondary-text">
            {modelCount > 0 ? `${modelCount}개 모델이 구성됨` : '구성된 모델 없음'}
          </p>
        </div>
      ...
      {expanded ? (
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] font-medium text-muted-text">구성 참고</span>
              ...
              <p className="text-[11px] leading-5 text-muted-text">
                기능 태그는 구성 참고용일 뿐이며, 런타임 기능이 검증되었음을 의미하지는 않습니다.
              </p>
            </div>
          ) : null}
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? '로컬 Ollama는 비워도 됩니다' : '여러 Key는 쉼표로 구분'}
          />
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {discoveryState?.status === 'loading' ? '불러오는 중...' : '모델 가져오기'}
              </Button>
              ...
              >
                {discoveryState?.text || '`/models`를 지원하는 OpenAI Compatible 채널은 모델을 자동으로 가져올 수 있습니다.'}
              </span>
            </div>
            {discoveryState?.hint ? (
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? '목록에 없는 커스텀 모델명이 있다면 수동으로 추가할 수 있으며, 저장 형식은 여전히 쉼표 구분입니다.'
                  : '채널이 자동 탐색을 지원하지 않거나 요청이 실패하면 모델 목록을 직접 수동으로 입력할 수 있습니다.'
              }
            />
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    기본 연결 테스트는 모델 목록의 첫 번째 모델을 사용합니다: {selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="flex items-center gap-1.5">
                  <p className="text-[11px] font-medium text-muted-text">런타임 기능 검사(선택 사항)</p>
                  <SettingsHelpButton
                ...
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  수동으로 실행할 때만 실제 LLM 요청을 보냅니다. 여러 항목 선택 시 20~40초가 걸릴 수 있습니다.
                </p>
              </div>
              <Button
                type="button"
...
function buildModelOptions(models: string[], selectedModel: string, autoLabel: string): Array<{ value: string; label: string }> {
  ...
  if (selectedModel && !models.includes(selectedModel)) {
    options.push({ value: selectedModel, label: `${selectedModel}(현재 구성)` });
  }
...
const LLM_ERROR_LABELS: Record<string, string> = {
  ...
  unsupported_protocol: '지원하지 않는 프로토콜',
  capability_unsupported: '지원하지 않는 기능',
  skipped: '건너뜀',
};

const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: 'API Key가 올바른지, 불필요한 공백이 없는지, 현재 채널에 추가 조직/프로젝트 권한이 필요한지 확인하세요.',
  timeout: '재시도할 수 있습니다. 계속 시간 초과가 발생하면 Base URL, 네트워크 프록시, 서비스 제공업체 가용 영역 또는 로컬 방화벽을 확인하세요.',
  quota: '잔액, 요금제 할당량, RPM/TPM 속도 제한 또는 동시성 설정을 확인하고, 필요한 경우 나중에 다시 시도하세요.',
  model_not_found: '모델 이름이 채널 프로토콜과 일치하는지 확인하고, 먼저 "모델 가져오기"로 해당 채널의 실제 사용 가능한 모델 목록을 확인하세요.',
  empty_response: '채널은 연결되었지만 본문이 반환되지 않았습니다. 호환 모델로 전환하거나 추가 응답 모드를 끈 후 다시 테스트해 보세요.',
  network_error: 'Base URL, 프록시, TLS/인증서, 중계 게이트웨이 또는 로컬 네트워크 정책을 확인하고, 나중에 다시 시도해 보세요.',
  invalid_config: '프로토콜, Base URL, API Key, 모델 구성을 먼저 채운 후 원클릭 테스트를 실행하세요.',
  unsupported_protocol: '현재 OpenAI Compatible / DeepSeek 채널에만 자동 모델 탐색이 제공됩니다. 모델 목록을 수동으로 관리하세요.',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'API Key가 비어 있거나 쉼표로 구분한 후 사용 가능한 Key가 없습니다. 유효한 Key를 하나 이상 입력한 후 테스트하세요.',
  api_key_rejected: '서비스 제공업체가 현재 API Key를 거부했습니다. Key, 조직/프로젝트 권한, 지역, 계정 상태를 확인하세요.',
  rate_limit: '서비스 제공업체가 RPM/TPM 또는 동시성 제한을 트리거했습니다. 요청 빈도를 낮추거나 나중에 다시 시도하세요.',
  insufficient_balance: '서비스 제공업체가 잔액, 청구 또는 할당량 부족을 반환했습니다. 계정 잔액과 요금제 상태를 확인하세요.',
  quota_exceeded: '서비스 제공업체가 할당량 소진을 반환했습니다. 계정 요금제, 잔여량, 프로젝트 할당량을 확인하세요.',
  provider_blocked: '요청이 서비스 제공업체 또는 중계 게이트웨이에 의해 차단되었습니다. 계정 리스크 관리, 지역 제한, 모델 권한, 중계 게이트웨이 정책, 콘텐츠 안전 정책 또는 요청 출처 제한을 확인하세요.',
  dns_error: '도메인 해석에 실패했습니다. Base URL 도메인, 네트워크 프록시, DNS 구성을 확인하세요.',
  tls_error: 'TLS/인증서 핸드셰이크에 실패했습니다. HTTPS 인증서, 중계 게이트웨이 또는 회사 프록시 정책을 확인하세요.',
  connection_refused: '대상 서비스가 연결을 거부했습니다. Base URL 포트, 서비스 프로세스, 방화벽 구성을 확인하세요.',
  model_access_denied: '현재 계정으로 해당 모델을 사용할 수 없습니다. 모델이 활성화되었는지, 계정에서 볼 수 있는지, 모델이 비활성화되지 않았는지 확인하세요.',
  provider_prefix_mismatch: '모델의 provider 접두사가 현재 채널과 일치하지 않습니다. 모델 이름이 해당 채널의 OpenAI-compatible 라우팅을 사용해야 하는지 확인하세요.',
  capability_unsupported: '현재 모델 또는 호환 계층이 해당 기능을 지원하지 않습니다. 기본 텍스트 연결에는 영향을 주지 않으며, 모델을 바꾸거나 해당 기능 의존성을 끌 수 있습니다.',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? '해당 채널이 반환한 /models 응답 형식이 호환되지 않습니다. 모델 목록을 수동으로 입력하세요.'
      : '반환 구조가 예상과 일치하지 않습니다. 해당 채널이 Chat Completions API와 호환되는지 확인하세요.';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return '해당 채널의 /models API가 사용 가능한 모델 ID를 반환하지 않았습니다. Base URL이 호환되는 모델 목록 API를 가리키는지 확인하거나, 모델 목록을 수동으로 입력하세요.';
  }
...
function buildLlmTestHint(result: {
  ...
  const scopeInfo = '기본 연결 테스트는 기본적으로 모델 목록의 첫 번째 모델만 테스트합니다.';
  ...
  const modelActionHint = shouldSuggestModelListChange
    ? '해당 모델을 사용할 수 없으면 모델 순서를 조정하거나 사용할 수 없는 모델을 제거한 후 다시 시도하세요.'
    : '';
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: '채널 이름은 비워둘 수 없으며 영문자, 숫자, 밑줄만 포함할 수 있습니다.' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes/non-Hermes route는 메인 생성 모델 또는 대체 모델로 지원되지 않습니다. 순수 Hermes 또는 순수 비-Hermes route를 선택하세요.' });
        return;
      }
      ...
      if (nonCanonicalRouteAlias) {
        setSaveMessage({ type: 'local-error', text: '현재 런타임 모델이 비표준 route alias를 사용하고 있습니다. 드롭다운에서 표준 모델을 다시 선택하세요.' });
        return;
      }
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '현재 메인 모델이 활성화된 채널의 모델 목록에 없습니다. 다시 선택하세요.' });
        return;
      }
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '현재 Agent 메인 모델에 Agent-safe 비-Hermes deployment가 없습니다. 다시 선택하세요.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: '현재 Vision 모델은 Hermes deployment를 포함할 수 없습니다. 순수 비-Hermes route를 다시 선택하세요.' });
        return;
      }
    }
    ...
    try {
      ...
      setSaveMessage({ type: 'success', text: managesRuntimeConfig ? 'AI 구성이 저장되었습니다' : '채널 구성이 저장되었습니다' });
    } catch (error: unknown) {
      setSaveWarnings([]);
    ...
    try {
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: result.success ? 'success' : 'error',
          text: result.success
            ? `모델 ${result.models.length}개를 불러왔습니다${result.latencyMs ? ` · ${result.latencyMs} ms` : ''}`
            : buildLlmFailureText(result),
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: 'error',
          text: parsed.message || '모델 불러오기 실패',
          hint: undefined,
          models: previous[channel.id]?.models || [],
        },
      }));
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-foreground">AI 모델 구성</h3>
            <Badge variant="info" className="settings-accent-badge">채널 관리</Badge>
          </div>
          <p className="text-xs text-muted-text">
            서비스 제공업체 채널을 추가하면 모델 목록을 자동으로 가져와 다중 선택할 수 있으며, 수동 입력도 계속할 수 있습니다. 구성은 .env 파일에 자동으로 동기화됩니다.
          </p>
        </div>
      ...
      {!isCollapsed ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-secondary-text">먼저 프리셋 서비스 제공업체를 선택한 후 원클릭으로 구성 초안을 만드세요.</p>
              </div>
          ...
          <div className="space-y-2">
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                ...
                <p className="mt-1 text-xs text-muted-text">서비스 제공업체 프리셋을 선택하고 "채널 추가"를 클릭하면 구성이 시작됩니다.</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-muted-text">메인 모델, 대체 모델, Vision, Temperature가 런타임 구성에 직접 기록됩니다.</p>
                </div>
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  모델 출력의 무작위성을 제어합니다. 0은 결정적 출력, 2는 최대 무작위성이며 0.7을 권장합니다.
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  활성화된 채널을 하나 이상 추가하고 모델을 입력해야 아래의 메인 모델 / 대체 모델 / Vision 옵션이 나타납니다.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(availableModels, runtimeConfig.primaryModel, '자동(첫 번째 사용 가능한 모델 사용)')}
                      disabled={busy}
                      placeholder=""
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-secondary-text">
                      대체 모델은 메인 모델이 실패할 때만 사용됩니다. 메인 모델은 대체 모델에 중복 추가되지 않습니다.
                    </p>
                  </div>
            ...
            <InlineAlert
              variant="warning"
              message="고급 모델 라우팅 YAML이 구성되어 있음을 감지했습니다. 여기서는 채널 항목과 기본 연결 정보만 관리합니다. 런타임 메인 모델 / 대체 모델 / Vision / Temperature는 여전히 아래 일반 필드로 결정됩니다. YAML 파싱에 성공하면 YAML의 라우팅 및 사용 가능한 모델 선언이 우선하며, 이 구성은 YAML 파일 자체를 덮어쓰지 않습니다."
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              {isSaving ? '저장 중...' : managesRuntimeConfig ? 'AI 구성 저장' : '채널 구성 저장'}
            </Button>
