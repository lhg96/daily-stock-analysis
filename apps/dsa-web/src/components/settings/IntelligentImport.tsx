...
function getConfidenceMeta(confidence: 'high' | 'medium' | 'low', language: UiLanguage) {
  if (confidence === 'high') {
    return { label: language === 'en' ? 'High' : '높음', badge: 'success' as const };
  }
  if (confidence === 'low') {
    return { label: language === 'en' ? 'Low' : '낮음', badge: 'warning' as const };
  }
  return { label: language === 'en' ? 'Medium' : '보통', badge: 'default' as const };
}
