...
const MARKET_BADGE_CONFIG = {
  ...
  HK: { label: '홍콩주', className: 'border-success/25 bg-success/10 text-success' },
  US: { label: '미국주', className: 'border-cyan/25 bg-cyan/10 text-cyan' },
  JP: { label: '일본주', className: 'border-indigo-500/25 bg-indigo-500/10 text-indigo-500' },
  KR: { label: '한국주', className: 'border-rose-500/25 bg-rose-500/10 text-rose-500' },
...
function MatchTypeBadge({ matchType }: { matchType: string }) {
  const configMap = {
    ...
    fuzzy: { label: '유사', className: 'border-border/55 bg-elevated/75 text-muted-text' },
  };
