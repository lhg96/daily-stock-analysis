...
export const useStockPoolStore = create<StockPoolState>((set, get) => ({
  ...
  submitAnalysis: async (options) => {
    ...
    if (!stockCodeInput) {
      set({ inputError: '주식 코드를 입력해 주세요', duplicateError: null });
      return;
    }

    if (selectionSource !== 'autocomplete' && isObviouslyInvalidStockQuery(stockCodeInput)) {
      set({ inputError: '유효한 주식 코드 또는 주식 이름을 입력해 주세요', duplicateError: null });
      return;
    }
    ...
    try {
      ...
      if (error instanceof DuplicateTaskError) {
        set({
          duplicateError: `${error.stockCode} 분석 중입니다. 완료될 때까지 기다려 주세요`,
        });
        return;
      }
