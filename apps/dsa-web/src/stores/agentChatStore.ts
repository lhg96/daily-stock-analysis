...
function streamFailureFallback(event: StreamFailureEvent, defaultMessage: string): string {
  return event.backend === 'codex_app_server'
    ? 'Codex Agent가 이번 종목 문의를 완료할 수 없습니다. Agent 설정의 실행 상태를 확인해 주세요.'
    : defaultMessage;
}
  ...
  startStream: async (payload, meta) => {
    ...
    try {
      ...
      const protocolError = (message: string) => createParsedApiError({
        title: '요청이 수락되지 않음',
        message: 'Agent가 이번 질문을 수락하지 않았습니다. 현재 내용을 유지한 후 다시 시도해 주세요.',
        rawMessage: message,
      ...
      const processLine = (line: string) => {
        ...
        if (event.type === 'done') {
          ...
          if (doneEvent.success === false) {
            throw getStreamFailureError(
              doneEvent,
              streamFailureFallback(doneEvent, '대형 언어 모델 호출 오류. API Key 구성을 확인해 주세요'),
            );
          }
        ...
        if (event.type === 'error') {
          ...
          throw getStreamFailureError(
            failureEvent,
            streamFailureFallback(failureEvent, '분석 오류'),
          );
        }
