    ...
    try {
      ...
      if (!validateResult.valid) {
        setSaveError(createParsedApiError({
          title: '구성 검증 실패',
          message: '먼저 양식 오류를 수정한 후 저장해 주세요.',
          rawMessage: '구성 검증에 실패했습니다. 먼저 양식 오류를 수정해 주세요.',
          category: 'http_error',
        }));
        setRetryAction('save');
        return {
      ...
      const warningText = updateResult.warnings?.length
        ...
        : '';
      setToast({ type: 'success', message: `구성이 업데이트되었습니다${warningText}` });
      return { success: true };
    } catch (error: unknown) {
      if (error instanceof SystemConfigValidationError) {
        ...
        setSaveError(createParsedApiError({
          title: '구성 버전 충돌',
          message: `${error.message}. 구성을 다시 로드해 주세요.`,
