...
_LANES = [
    {"id": "entry", "label": "진입", "order": 1},
    {"id": "data_source", "label": "데이터 소스", "order": 2},
    {"id": "analysis", "label": "분석 엔진", "order": 3},
    {"id": "artifact", "label": "산출물", "order": 4},
]
...
def build_task_run_flow_snapshot(
    ...
    *,
    ...
    _append_edge(edges, "request", "task_queue", "control", flow_status, label="제출")
    ...
    if flow_status in {"pending", "running", "cancel_requested"}:
        ...
        _append_edge(edges, "task_queue", "analysis_pipeline", "control", flow_status, label="스케줄링")
    else:
...
def build_history_run_flow_snapshot(
    ...
    *,
    ...
    _put_node(
        ...
        started_at=created_at,
        ended_at=created_at,
        message="작업 완료 — 기록 저장됨",
    )
    _append_edge(edges, "request", "task_queue", "control", "success", label="제출")
    _append_event(
        events,
        "task_completed",
        node_id="task_queue",
    ...
    if not any(edge["to"] == "context_pack" for edge in edges):
        _append_edge(edges, "task_queue", "context_pack", "data", context_status, label="입력")
    ...
    if last_analysis_node is None:
        _put_node(
            ...
            message="LLM 진단 정보 없음",
        )
        _append_edge(edges, "context_pack", "llm", "data", "unknown", label="생성")
    ...
    if last_artifact_node is None:
        ...
        _append_edge(edges, last_analysis_node, "history_save", "data", "success", label="저장")
    ...
    if notification_count == 0:
        ...
        _append_edge(edges, last_artifact_node, "notification", "control", "unknown", label="알림")
    ...
    for index, raw_run in enumerate(provider_runs, start=1):
        ...
        if previous:
            ...
            _append_edge(
                ...
                edge_kind,
                status,
                label="대체" if edge_kind == "fallback" else "재시도",
                message=_safe_text(run.get("fallback_from") or run.get("fallback_to"), max_length=120),
            )
        else:
            _append_edge(edges, "task_queue", node_id, "control", status, label="호출")
        ...
        _append_event(
            ...
            timestamp=timestamp,
            severity=severity,
            title=f"{label}{'성공' if success else '실패'}",
            message=message,
            metadata={
                "provider": provider,
                "data_type": data_type,
...
def _append_context_blocks(
    ...
    for block in _as_list(overview.get("blocks")):
        ...
        if provider_node_id:
            _append_edge(edges, provider_node_id, node_id, "data", status, label="입력")
        else:
            _append_edge(edges, "task_queue", node_id, "data", status, label="입력")
        _append_edge(edges, node_id, "context_pack", "data", status, label="조립")
        if status != "success":
            _append_event(
                events,
                ...
                title=f"{block_map.get('label') or key} 입력 상태",
                message=_context_block_message(block_map),
                metadata={
                    "block_key": key,
...
def _append_llm_runs(
    ...
    for index, raw_run in enumerate(llm_runs, start=1):
        ...
        _append_edge(edges, previous_node_id, node_id, edge_kind, status, label="생성")
        _append_event(
            events,
            "llm_run",
            node_id=node_id,
            ...
            title=f"LLM {'성공' if success else '실패'}",
            message=message,
            metadata={
                "provider": provider,
                "model": model,
...
def _append_history_runs(
    ...
    *,
    ...
    for index, raw_run in enumerate(history_runs, start=1):
        ...
        message = "보고서 기록 저장됨" if success else f"보고서 기록 저장 실패: {_safe_text(run.get('error_message_sanitized'), max_length=160) or '알 수 없는 오류'}"
        _put_node(
            nodes,
            node_id,
            lane="artifact",
        ...
        _append_edge(edges, previous_node_id, node_id, "data", status, label="저장")
        _append_event(
            events,
            "history_run",
            node_id=node_id,
            timestamp=timestamp,
            severity="success" if success else "danger",
            title="기록 저장 성공" if success else "기록 저장 실패",
            message=message,
            metadata={
                "metadata_saved": run.get("metadata_saved"),
                "analysis_history_id": run.get("analysis_history_id"),
...
def _append_notification_runs(
    ...
    *,
    ...
    for index, raw_run in enumerate(notification_runs, start=1):
        ...
        _append_edge(edges, anchor_node_id, node_id, "control", status, label="알림")
        _append_event(
            events,
            "notification_run",
            node_id=node_id,
            ...
            title="알림 전송 성공" if status == "success" else ("알림 건너뜀" if status == "skipped" else "알림 실패"),
            message=message,
            metadata={
                "channel": channel,
                "status": raw_status,
...
def _put_skeleton_tail(
    ...
    *,
    ...
    _append_edge(edges, anchor_node_id, "context_pack", "data", downstream_status, label="입력")
    _append_edge(edges, "context_pack", "llm", "data", downstream_status, label="생성")
    _append_edge(edges, "llm", "history_save", "data", downstream_status, label="저장")
    _append_edge(edges, "history_save", "notification", "control", downstream_status, label="알림")
...
def _append_task_events(events: List[Dict[str, Any]], task: Any, flow_status: str) -> None:
    _append_event(
        ...
        severity="info",
        title="작업 생성됨",
        message=getattr(task, "message", None) or "작업이 큐에 추가됨",
    )
    if getattr(task, "started_at", None):
        _append_event(
            ...
            severity="info",
            title="작업 실행 시작",
            message=getattr(task, "message", None) or "작업 실행 중",
        )
    if flow_status == "failed":
        ...
        _append_event(
            ...
            timestamp=_datetime_to_iso(getattr(task, "completed_at", None)),
            severity="warning",
            title="작업 취소" if flow_status == "cancelled" else "작업 요청 취소",
        ...
        _append_event(
            ...
            severity="success",
            title="작업 완료",
            message=getattr(task, "message", None) or "분석 완료",
        )
            ...
            if provider_data_type and provider_run:
                ...
                if previous_provider:
                    ...
                    _append_edge(
                        ...
                        label="대체" if edge_kind == "fallback" else ("재시도" if edge_kind == "retry" else "호출"),
                    )
                else:
                    _append_edge(edges, "task_queue", node_id, "control", nodes[node_id].get("status", "unknown"), label="호출")
                last_provider_node_by_type[provider_data_type] = (node_id, provider_run)
            elif event_type in {"llm_run", "llm_run_started"}:
                anchor = "analysis_pipeline" if "analysis_pipeline" in nodes else "task_queue"
                _append_edge(edges, anchor, node_id, "data", nodes[node_id].get("status", "unknown"), label="생성")
                last_llm_node = node_id
            elif event_type == "history_run":
                anchor = last_llm_node or ("analysis_pipeline" if "analysis_pipeline" in nodes else "task_queue")
                _append_edge(edges, anchor, node_id, "data", nodes[node_id].get("status", "unknown"), label="저장")
                last_history_node = node_id
                ...
                _append_edge(edges, anchor, node_id, "control", nodes[node_id].get("status", "unknown"), label="알림")
    ...
    events.append(
        {
            ...
            "title": _safe_text(event.get("title"), max_length=100) or "실행 이벤트",
...
def _context_pack_message(overview: Optional[Dict[str, Any]]) -> str:
    if not overview:
        return "AnalysisContextPack 개요 미기록"
    counts = overview.get("counts")
    if isinstance(counts, Mapping):
        available = counts.get("available", 0)
        return f"입력 컨텍스트 조립 완료, 사용 가능 블록 {available}"
    return "입력 컨텍스트 조립 완료"


def _context_block_message(block: Dict[str, Any]) -> str:
    status = str(block.get("status") or "")
    if status == "available":
        return "이번 분석 입력에 포함됨"
    if status == "fallback":
        return "이번 분석 입력은 대체 데이터 사용"
    if status == "partial":
        return "이번 분석 입력은 일부만 사용 가능"
    if status == "stale":
        return "이번 분석 입력은 만료 데이터 사용"
    if status == "estimated":
        return "이번 분석 입력은 추정 데이터 사용"
    if status == "fetch_failed":
        return "입력 블록 수집 실패"
    if status == "missing":
        reasons = _as_list(block.get("missing_reasons"))
        reason = _safe_text(reasons[0], max_length=120) if reasons else None
        return f"이번 분석 입력 미포함: {reason}" if reason else "이번 분석 입력 미포함"
    if status == "not_supported":
        return "현재 시장 또는 경로에서 이 입력 블록을 지원하지 않음"
    return f"입력 블록 상태: {status or 'unknown'}"


def _provider_run_message(label: str, provider: str, run: Dict[str, Any], *, success: bool) -> str:
    if success:
        record_count = _safe_int(run.get("record_count"))
        suffix = f", 반환 {record_count}건" if record_count is not None else ""
        return f"{label} {provider} 성공{suffix}"
    error = _safe_text(run.get("error_message_sanitized") or run.get("error_type"), max_length=160)
    return f"{label} {provider} 실패: {error or '알 수 없는 오류'}"
...
def _llm_run_message(model: Optional[str], run: Dict[str, Any], *, success: bool) -> str:
    ...
    if success:
        if run.get("fallback_model"):
            return f"LLM {display_model} 성공 (모델 전환 발생)"
        return f"LLM {display_model} 성공"
    error = _safe_text(run.get("error_message_sanitized") or run.get("error_type"), max_length=160)
    return f"LLM {display_model} 실패: {error or '알 수 없는 오류'}"


def _notification_run_message(channel: str, run: Dict[str, Any], status: str) -> str:
    if status == "success":
        return f"{channel} 알림 전송 성공"
    if status == "skipped":
        return f"{channel} 알림 건너뜀"
    if status == "failed":
        error = _safe_text(run.get("error_message_sanitized") or run.get("status"), max_length=160)
        return f"{channel} 알림 실패: {error or '알 수 없는 오류'}"
    return f"{channel} 알림 결과 알 수 없음"
...
def _task_status_message(status: str) -> str:
    return {
        ...
        "success": "작업 완료",
        "failed": "작업 실패",
        "cancel_requested": "작업 요청 취소",
        "cancelled": "작업 취소됨",
    }.get(status, "작업 상태 알 수 없음")
